...
const FoodOrderReport = () => {
  ...
  const [month, setMonth] = useState(1); // Default month set to 1
  ...
  const fetchFoodOrders = async (month) => {
      ...
      const displayFoodOrders = async (month) => {
        if (month < 10) {
          ...
          const notApplicableData = Array.from({ length: daysInMonth }, (_, index) => ({
            date: `${index + 1}-${month}-2024`,
            status: {
              breakfast: 'Data not found',
              lunch: 'Data not found',
              dinner: 'Data not found',
            },
            dailyFine: 0,
          }));
          setReport(notApplicableData);
          setTotalFine(0);
          return;
        }
